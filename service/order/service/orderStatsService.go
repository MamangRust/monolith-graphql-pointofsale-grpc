...
import (
	...
	sharederrorhandler "github.com/MamangRust/monolith-graphql-pointofsale-shared/errorhandler"
...
func (s *orderStatsService) FindMonthlyTotalRevenueById(ctx context.Context, req *requests.MonthTotalRevenueOrder) ([]*db.GetMonthlyTotalRevenueByIdRow, error) {
	...
	// NOTE: OrderStatsByIdCache interface was not provided.
	// Calling repository directly without caching for this specific method.
	res, err := s.orderStatsRepository.GetMonthlyTotalRevenueById(ctx, req)
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[[]*db.GetMonthlyTotalRevenueByIdRow](
			s.logger,
			order_errors.ErrFailedFindMonthlyTotalRevenueById.WithInternal(err),
			method,
			span,
			zap.Error(err),
		)
	...
	logSuccess("Successfully fetched monthly total revenue by order id", zap.Int("order_id", req.OrderID), zap.Int("year", req.Year), zap.Int("month", req.Month))
	return res, nil
}

func (s *orderStatsService) FindYearlyTotalRevenueById(ctx context.Context, req *requests.YearTotalRevenueOrder) ([]*db.GetYearlyTotalRevenueByIdRow, error) {
	...
	// NOTE: OrderStatsByIdCache interface was not provided.
	// Calling repository directly without caching for this specific method.
	res, err := s.orderStatsRepository.GetYearlyTotalRevenueById(ctx, req)
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[[]*db.GetYearlyTotalRevenueByIdRow](
			s.logger,
			order_errors.ErrFailedFindYearlyTotalRevenueById.WithInternal(err),
			method,
			span,
			zap.Error(err),
		)
	...
	logSuccess("Successfully fetched yearly total revenue by order id", zap.Int("order_id", req.OrderID), zap.Int("year", req.Year))
	return res, nil
}
