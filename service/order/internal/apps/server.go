...
func NewServer(cfg *server.Config) (*server.GRPCServer, error) {
	...
	cashierClient := cashierpb.NewCashierQueryServiceClient(cashierConn)
	merchantClient := merchantpb.NewMerchantQueryServiceClient(merchantConn)
	productClient := productpb.NewProductQueryServiceClient(productConn)
	productCommandClient := productpb.NewProductCommandServiceClient(productConn)
	orderItemClient := orderitempb.NewOrderItemServiceClient(orderItemConn)

	repos := repository.NewRepositories(srv.DB, cashierClient, merchantClient, productClient, productCommandClient, orderItemClient)
	...
	srv.RegisterServices = func(gs *grpc.Server) {
		orderpb.RegisterOrderQueryServiceServer(gs, handlers.OrderQuery)
		orderpb.RegisterOrderCommandServiceServer(gs, handlers.OrderCommand)
		orderpb.RegisterOrderStatsServiceServer(gs, handlers.OrderStats)
	}
