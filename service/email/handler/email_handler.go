...
func (h *EmailHandler) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	...
	// message is redelivered on the next rebalance; malformed and invalid
	// messages are committed as poison so they cannot block the partition.
	// Phase 2 (event contract): the payload must be a standard email envelope
	// (event_id, schema_version=1, event_type + email/subject/body). Typed
	// struct parsing never panics; invalid envelopes never reach SMTP.
	...
	// send succeeds. This replaces the previous in-memory deduper, which lost
	// all state on restart and could not dedupe events across rebalances.
