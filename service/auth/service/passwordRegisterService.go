...
import (
	...
	emails "github.com/MamangRust/monolith-graphql-pointofsale-pkg/email"
...
func (s *passwordResetService) ForgotPassword(ctx context.Context, email string) (bool, error) {
	...
	// Phase 6 — transactional outbox: the reset-token insert and the outbox event
	// commit in a single database transaction so a crash cannot lose the reset
	// email. Without a pool (tests/local) this falls back to direct Kafka.
	if s.pool != nil {
